    ...
    <main class="lab-grid" :class="{ 'code-panel-open': isCodePanelOpen }">
      <aside class="catalog-panel">
        ...
        <div class="catalog-guide">
          <span>操作约定</span>
          <p>点位单击完成；线和多边形双击结束；规则图形通过两次单击确定。保留后的控制点可以拖拽编辑。</p>
        </div>
      </aside>
...
.code-panel-body { box-sizing: border-box; min-width: 346px; min-height: 0; height: 100%; padding: 20px 18px 16px; display: flex; flex-direction: column; overflow: hidden; visibility: visible; opacity: 1; transform: translateX(0); transition: opacity 190ms ease 150ms, transform 230ms cubic-bezier(0.22, 1, 0.36, 1) 120ms, visibility 0s linear 0s; }
